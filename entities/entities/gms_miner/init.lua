...
function ENT:Mine( rock )
	...
	if math.random(100) == 1 then
		RunConsoleCommand("ulx", "psay", self:GetNWString("Owner"), "(Automated) Your miner broke!")
		self.broken = true
	end
end
...
function ENT:Think()
	if self.broken then
		...
		if self.fuel > 0 then
			...
			if self.rockdead == false then 
			self.fuel = self.fuel - 1
			self.rockdead = nil
			end
		end
		if self.fuel <= 0 then
			if self.fuelmsgcooldown == false then
				RunConsoleCommand("ulx", "psay", self:GetNWString("Owner"), "(Automated) Your miner is out of fuel!")
				self.fuelmsgcooldown = true
			end
		end
	end
...
function ENT:StartTouch(ent2)
	...
	if ent2:GetClass() == "gms_resourcedrop" then
		if ( !ent2.rType == "coal" ) or ( !ent2.rType == "charcoal" ) then return end
