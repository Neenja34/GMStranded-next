...
net.Receive("gm_tv_play", function()
    ...
    if !IsValid(ent) or !ent.PlayVideo then
...
net.Receive("gm_tv_playt", function()
    ...
    if !IsValid( ent ) or !ent.PlayTwitch then
        local function retry()
            ent = ents.GetByIndex(ent_index)
...
function ENT:ReloadPlayer()
	...
	self:Reload()
	self.m_pnlWebPage:OpenURL("http://neenja.nn.pe/tvplayer/player.php")
end
...
function ENT:LoadVideo()
	self.m_pnlWebPage:RunJavascript([[jQuery("#youtube-video-player").tubeplayer("stop");]])
	self.m_pnlWebPage:RunJavascript('jQuery("#youtube-video-player").tubeplayer("play", {id: "' .. string.JavascriptSafe(self.m_strPlayingID) .. '", time: ' .. self.m_intPlayTime + (RealTime() - self.m_intStartPlay) .. '});')
	self.m_bVideoLoaded = true
end
...
function ENT:Draw()
	...
	if viewNorm:Dot(self:GetAngles():Forward() * - 1) <= 0 then return end

	if self:GetPos():DistToSqr(EyePos()) > 27457600 then return end

	if !self.m_intLastFrame or CurTime() > self.m_intLastFrame then
		self.m_pnlWebPage:UpdateHTMLTexture()
		self.m_intLastFrame = CurTime() + (1 / 24)
	end

	self.m_matHTML = self.m_pnlWebPage:GetHTMLMaterial()
	if !self.m_matHTML then return end
...
function Panel:Init()
	...
	self.m_pnlStopBtn.Think = function(btn)
		...
		if !self:GetEntity():HasPlayer() then return end
		btn:SetDisabled(!self:GetEntity():IsPlayingVideo())
	end
