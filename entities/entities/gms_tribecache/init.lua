...
function ENT:SetResourceDropInfo( rType, rAmt )
	...
	if !GAMEMODE.Tribes.tblTribes[ self.tribeid ].cachecontents then GAMEMODE.Tribes.tblTribes[ self.tribeid ].cachecontents = {} end
...
function ENT:Use( ply )
	...
	if ( GAMEMODE.Tribes:GetPlayersTribe( ply ) ~= self.tribeid ) then
...
function ENT:StartTouch( ent2 )
	...
	if !IsValid( owner2 ) then return end
	if !IsValid( owner1 ) then return end

	if isbool( owner2 ) then return end

	if owner1:Team() ~= owner2:Team() then return end
