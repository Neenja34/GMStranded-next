...
elseif CLIENT then
	...
	local function DrawHat( ent )
		if !IsValid( ent.m_entEquippedHat ) or (ent:Health() <= 0 and ent:IsPlayer() and !IsValid(ent:GetRagdollEntity())) then return end
		...
		if scale.x < 0 then swap = !swap end
		if scale.y < 0 then swap = !swap end
