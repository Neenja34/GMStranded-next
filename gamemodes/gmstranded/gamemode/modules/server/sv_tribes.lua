...
function GM.Tribes:DeleteTribe( tr_id )
	if !self:IsValid( tr_id ) then return end
...
net.Receive( "tribes_cl_createtribe", function( l, ply )
	...
	if GAMEMODE.Tribes.tblTribes[ tr_id ] ~= nil then
		ply:SendMessage( "Nice try! Go play fortnite instead!", 60, Color( 255, 50, 50, 255 ) )
		SGS_Log( "!!!!!!!!!!! " .. ply:Nick() .. " [" .. ply:SteamID() .. "] is trying to create a tribe with already existing ID" )
	else
...
net.Receive( "tribes_cl_jointribe", function( l, ply )
	...
	if !GAMEMODE.Tribes:IsValid( t_id ) then return end
...
function GM.Tribes:PlaceInTribe( ply, t_id )
	...
	if !self:IsValid( t_id ) then return end
...
function GM.Tribes:JoinTribe( ply, t_id, pass )
	...
	if self:GetPermissionType( t_id ) == 1 then
		if ( self.tblTribes[ t_id ].perms.password ~= pass ) then
	...
	if self:GetPermissionType( t_id ) == 2 then
		if !self:RequestJoinTribe( ply, t_id ) then
	...
	if !ply.tribe_lastjoin then ply.tribe_lastjoin = CurTime() - 130 end
...
net.Receive( "tribes_cl_kickplayer", function( l, ply )
	...
	local t_id = tbl.t_id
	local p_id = tbl.p_id
	if !GAMEMODE.Tribes:PlayerOwnsTribe( ply ) then return end
	if ( GAMEMODE.Tribes:GetPlayersTribe( ply ) ~= t_id ) then return end
...
function GM.Tribes:SendTribeInvitation( ply, inv_ply, t_id )
	...
	if !IsValid( inv_ply ) then return end
	if !self:IsValid( t_id ) then return end
	if self:RequestJoinTribe( inv_ply, t_id ) then return end
	if ( self:GetPlayersTribe( ply ) ~= t_id ) then return end
...
function GM.Tribes:DisbandTribe( ply )
	if !self:PlayerOwnsTribe( ply ) then return end
	local current_tribe = self:GetPlayersTribe( ply )
	if current_tribe then
...
function GM.Tribes:DisbandTribeID( t_id )
	...
	if !self:IsValid( current_tribe ) then return end
	if current_tribe then
		local online_players = self:GetOnlinePlayersInTribe( current_tribe )
		for k, v in pairs( online_players ) do
			v:SetTeam( 10000 )
...
function GM.Tribes:UpgradeTribe( ply )
	...
	if !self.tblTribes[ t_id ].perma then
		self.tblTribes[ t_id ].perma = true
		self.tblTribes[ t_id ].level = 1
		self:NetworkTribe( nil, t_id )
		...
		ply:SendMessage( "Your tribe is already permanent!" , 60, Color( 255, 50, 50, 255 ) )
	end
...
function GM.Tribes:AddExp( t_id, amt )
	...
	if !self.tblTribes[ t_id ].exp then self.tblTribes[ t_id ].exp = 0 end
