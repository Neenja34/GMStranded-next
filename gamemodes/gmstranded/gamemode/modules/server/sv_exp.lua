...
function PlayerMeta:CalcMaxPlants()
	...
	if self:IsDonator() then
		self.maxplants = self.maxplants + 9
	end

	if self:GetUserGroup() == "bossdonator" then
		self.maxplants = self.maxplants + 3
	end

	if self:GetUserGroup() == "voiddonator" then
		self.maxplants = self.maxplants + 6
	end
	if self:GetUserGroup() == "bossadmin" then
		self.maxplants = self.maxplants + 3
	end

	if self:GetUserGroup() == "voidadmin" then
		self.maxplants = self.maxplants + 6
	end

	if self:GetUserGroup() == "developer" then
		self.maxplants = self.maxplants + 6
	end

	if self:GetUserGroup() == "bossmod" then
		self.maxplants = self.maxplants + 3
	end

	if self:GetUserGroup() == "voidmod" then
		self.maxplants = self.maxplants + 6
	end
...
function PlayerMeta:CalcMaxPets()
	...
	if self:IsDonator() then
		self.maxpets = self.maxpets + 10
	end

	if self:GetUserGroup() == "bossdonator" then
		self.maxpets = self.maxpets + 3
	end

	if self:GetUserGroup() == "voiddonator" then
		self.maxpets = self.maxpets + 6
	end
...
function PlayerMeta:AddExp( skill, amt, shared )
	...
	if amt == 0 then
	local clvl = self:GetLevel( skill )
	if clvl >= SGS.maxlevel then
	self:SetNWString(skill, "max")
	end
	return end
	local xp_string = ""
	if !shared then
		...
		if SGS.bonusexp[skill] and SGS.bonusexp[skill].enabled then
		...
		if GAMEMODE.Tribes:GetTribeLevel( self ) >= 1 then
			if !GAMEMODE.Tribes.ExpCaps then GAMEMODE.Tribes.ExpCaps = {} end
			if !GAMEMODE.Tribes.ExpCaps[ self:GetPlayerID() ] then GAMEMODE.Tribes.ExpCaps[ self:GetPlayerID() ] = 0 end
			if GAMEMODE.Tribes.ExpCaps[ self:GetPlayerID() ] < GAMEMODE.Tribes.PlayerExpCap then
				...
				if !GAMEMODE.Tribes.tblTribes[ GAMEMODE.Tribes:GetPlayersTribe( self ) ].expcontributions then GAMEMODE.Tribes.tblTribes[ GAMEMODE.Tribes:GetPlayersTribe( self ) ].expcontributions = {} end
				if !GAMEMODE.Tribes.tblTribes[ GAMEMODE.Tribes:GetPlayersTribe( self ) ].expcontributions[ self:SteamID() ] then GAMEMODE.Tribes.tblTribes[ GAMEMODE.Tribes:GetPlayersTribe( self ) ].expcontributions[ self:SteamID() ] = 0 end
...
function PlayerMeta:CheckSkillUnlock( skill, lvl )
	print( skill, lvl )
	if SGS.SkillUnlocks[skill] and SGS.SkillUnlocks[skill][lvl] then
...
function SGS_ConSetBonusExp( ply, _, args )
	if !SGS_IsRootAdmin( ply ) then return end
...
function SGS_AnnounceBonusExpOnJoin( ply )
	for k, v in pairs( SGS.skills ) do
		if SGS.bonusexp[v] and SGS.bonusexp[v].enabled then
