...
function SGS_ToolGemmingMenu()
	SGS.toolgemmingmenu = vgui.Create("sgs_toolgemmingmenu")
	SGS.toolgemmingmenu:MakePopup()
	SGS.toolgemmingmenu:SetVisible(true)
end

local toolgemmingPanel = {
	Init = function(self)
		local width = 425
		local height = 600
	...
	Paint = function(self, w, h)
		...
		draw.SimpleText("Tool Gemming Table", "SGS_RCacheTitles", wide / 2, 14, white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
}

function toolgemmingPanel:DrawFrame()
	local toolgemmingtable = vgui.Create("DPanel", self)
	toolgemmingtable:Dock(FILL)
	toolgemmingtable:SetPaintBackground(false)

	local CatList = vgui.Create("DPanelList", toolgemmingtable)
	...
	closeButton.DoClick = function()
		self:Remove()
		SGS.toolgemmingmenu:Remove()
	end
end
vgui.Register("sgs_toolgemmingmenu", toolgemmingPanel, "Panel")
