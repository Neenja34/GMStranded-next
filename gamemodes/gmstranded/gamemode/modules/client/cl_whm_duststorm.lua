...
function Dust:UpdateFadeIn()
	...
	if !self.m_intStartDustFadeTime then
...
function Dust:UpdateWindSounds( intScaler )
	...
	if self:Indoors() then
		...
		if !self.m_intOutdoorFadeStart then
...
function Dust:UpdateParticles( intScaler )
	...
	if !self.m_intLastPartUpdate then
	...
	for i = 1, math.Round(self.m_intMaxDustParts *intScaler) do
		...
		if !self:TracePartHull( pos, Size ) then
			continue
		end
		...
		if part then			
			...
			part:SetThinkFunction( function( p )
				if !self:TracePartHull( p:GetPos(), Size ) then
					p:SetDieTime( 0 )
				end
