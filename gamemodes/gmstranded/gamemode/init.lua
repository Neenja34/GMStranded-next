...
function SGS_PreCacheMessage()
	...
	util.AddNetworkString("cl_frompcachemeteoric")
	...
	util.AddNetworkString("cl_topcachemeteoric")
...
function SGS_SetUpTeams(ply)
	...
	if ply:IsUserGroup("voiddonator") then
	...
	if ply:IsUserGroup("bossdonator") then
		ply.allowedmodels = SGS.pmd
		net.Start("sgs_updateplayermodels")
			net.WriteString( "d" )
		net.Send( ply )
		...
		if ply:IsUserGroup("voidadmin") then
		ply.allowedmodels = SGS.pma
		net.Start("sgs_updateplayermodels")
			net.WriteString( "a" )
		net.Send( ply )
	...
	if ply:IsUserGroup("bossadmin") then
		ply.allowedmodels = SGS.pmd
		net.Start("sgs_updateplayermodels")
			net.WriteString( "a" )
		net.Send( ply )
		...
		if ply:IsUserGroup("voidmod") then
		ply.allowedmodels = SGS.pmd
		net.Start("sgs_updateplayermodels")
			net.WriteString( "d" )
		net.Send( ply )
	...
	if ply:IsUserGroup("bossmod") then
		ply.allowedmodels = SGS.pmd
		net.Start("sgs_updateplayermodels")
			net.WriteString( "d" )
		net.Send( ply )
...
function SGS_Spawn( ply, com, args )
	...
	--[[
	if item.xp == 0 then
		if !ply:GetAch( "betaprogram" ) then
...
function SGS_UseKey( ply, key )
	...
	if tr.HitWorld then
		...
		gmansounds = { "vo/gman_misc/gman_03.wav", "vo/gman_misc/gman_04.wav", "vo/gman_misc/gman_riseshine.wav" }
		
		if IsValid(tr.Entity) and tr.Entity == SGS.shopent4 then
			if makinganoise == "stfu" then
			ply:SendLua("SGS_ShopMenu2()")
			return end
			tr.Entity:EmitSound(table.Random(gmansounds), 60, 100)
			makinganoise = "stfu"
			ply:SendLua("SGS_ShopMenu2()")
			timer.Simple( 10, function() makinganoise = nil end )
			return
		end
...
function PlayerMeta:BottleWater()
	if self:WaterLevel() >= 1 or self:CheckWater(140) then
		...
		return
	elseif self:GetEyeTrace().Entity:GetClass() == "gms_sink" then
		SGS_BottleWater_Start( self, 2 )
		return
	end
	self:SendMessage("You must be standing in water/looking at a sink to bottle.", 60, Color(255, 0, 0, 255))
end

function PlayerMeta:DrinkBottleWater()

	if !self.resource[ "bottled_water" ] or self.resource[ "bottled_water" ] <= 0 then
...
function SGS_StargateReload( ply, text, public )
  if (string.sub(string.lower(text), 1, 11) == "!votereload") then
	if ( timer.Exists( "reallowedin" ) ) then
		ply:ChatPrint("Voting for stargate portals is allowed in " .. math.Round(timer.TimeLeft("reallowedin")) .. " seconds!" ) 
	return false end
	if ply:GetNWBool("votedstargate") == true then ply:ChatPrint("You have already voted!") return false end
	ply:SetNWBool("votedstargate",true)
	local totalplayers = #player.GetAll()
	local amtneeded = math.Round(totalplayers/3)
	if voteamt == nil then voteamt = 0 end
	if timer.Exists( "voteclose" ) == false or timer.Exists( "voteclose" ) == nil then
		timer.Create("voteclose",120,1, function()
			voteamt = 0
			timer.Create("reallowedin",120,1, function() end)
			for k, v in pairs(player.GetAll()) do
				v:ChatPrint("Stargate vote failed. Vote re-allowed in 2 minutes.")
				v:SetNWBool("votedstargate",nil)
			end
		end)
	end
	voteamt = voteamt + 1
	for k, v in pairs(player.GetAll()) do
		v:ChatPrint(ply:Nick() .. " has voted to reload stargate portals. (" .. voteamt .. "/" .. amtneeded .. ") - Vote closes in " .. math.Round(timer.TimeLeft("voteclose")) .. " seconds.")
	end
	if voteamt >= amtneeded then
		RunConsoleCommand("stargate_reload")
		for k, v in pairs(player.GetAll()) do
			v:ChatPrint("Stargate vote successful, reloading stargate. Vote re-allowed in 5 minutes.")
			v:SetNWBool("votedstargate",nil)
		end
		timer.Remove("voteclose")
		timer.Create("reallowedin",300,1, function() end)
		voteamt = 0
	end
  return false end
end
hook.Add( "PlayerSay", "SGS_StargateReload", SGS_StargateReload )
...
function PlayerMeta:Eat( food )
	...
	elseif food.sname == "package" then
		SGS_OpenEasterEgg(self)
	else
...
function SGS_AwardVoidTier( ply, tier )
	...
	elseif tier > 60 and tier <= 75 then
		ply:AddResource("hat_token", 1)
	elseif tier > 75 and tier <= 85 then
		ply:AddResource("cooked_shark", math.random(10))
	elseif tier > 85 and tier <= 100 then
...
function PlayerMeta:LevelUpSurvivalEffect()
	...
	local ent = ents.Create("gms_levelupsurvival")
		...
		-------------------------
		ent:Activate()
		self:EmitSound(Sound("sounds/levelup.mp3"), 65, math.random(70, 130))
end
...
function SGS_ConEat( ply, com, args )
		...
		if !ply.resource[ args[1] ] or ply.resource[ args[1] ] <= 0 then
...
function SGS_ConBuy( ply, com, args )
		...
		if !SGS_ReverseShopLookup( args[1] ) then
...
function SGS_ConBuy2( ply, com, args )
		...
		if !SGS_ReverseShopLookup2( args[1] ) then
			ply:SendMessage("Invalid item!", 60, Color(0, 255, 0, 255))
			return
		end
...
function PlayerMeta:Buy2( item )
	...
	if (self.resource[ "quest_token" ] or 0) < item.cost then
		can = false
		self:SendMessage("You do not have enough Quest Tokens.", 60, Color(255, 0, 0, 255))
		return
	end

	if can then
		self:SubResource( "quest_token", item.cost )
...
function SGS_ConSmelt( ply, com, args )
		...
		if !SGS_ReverseOreLookup( args[1] ) then
			ply:SendMessage("Invalid item!", 60, Color(0, 255, 0, 255))
			return
		end
...
function SGS_ConArcaneForge( ply, com, args )
		...
		if !SGS_ReverseStoneLookup( args[1] ) then
			ply:SendMessage("Invalid item!", 60, Color(0, 255, 0, 255))
			return
		end
...
function SGS_ConXMute( ply, com, args )
		...
		if !SGS_ReverseXMuteLookup( args[1] ) then
			ply:SendMessage("Invalid item!", 60, Color(0, 255, 0, 255))
			return
		end
...
function SGS_ConGrind( ply, com, args )
		...
		if !SGS_ReverseGrindLookup( args[1] ) then
			ply:SendMessage("Invalid item!", 60, Color(0, 255, 0, 255))
			return
		end
...
function SGS_ConSmith( ply, com, args )
		...
		if !SGS_ReverseToolLookup( args[1] ) then
...
function PlayerMeta:GemTool( tool )
	...
	if !self:HasTool( tool.entity2 ) then
		can = false
...
function SGS_ConCommandResetSkill( ply, com, args )
	...
	if SGS_IsRootAdmin(ply) or ply:IsUserGroup("admin") then
		...
		if !IsValid(target_ply) or !target_ply:IsPlayer() or !target_ply:IsConnected() then
...
local valid_cache_entities = { "gms_rcache", "gms_pcache", "gms_pcache2", "gms_pcache3", "gms_pcache4", "gms_pcacheboss", "gms_pcachemeteoric", "gms_tribecache",  }
function SGS_CacheValidCheck( ply, amt )
	...
	if !table.HasValue( valid_cache_entities, trace.Entity:GetClass() ) then
		return false
	end
...
net.Receive( "cl_topcache1", function( l, ply )
	...
	if !ent or !ent.enabled then return end
...
net.Receive( "cl_topcachemeteoric", function( l, ply )
	...
	net.Start( "UpdateCacheTable" )
		net.WriteTable( ent.POwner.pmeteoriccontents )
	net.Send( ply )
...
net.Receive( "cl_fromtribecache", function( l, ply )
	...
	if !ent or !ent:GetClass() == "gms_tribecache" then return end

	if(SPropProtection.PlayerUse(ply, ent) == false) then return end

	if !GAMEMODE.Tribes.tblTribes[ ent.tribeid ].cachecontents[res] then
...
net.Receive( "cl_fromrcache", function( l, ply )
	...
	if !ent or !ent:GetClass() == "gms_rcache" then return end
	if !ent.contents then return end

	if(SPropProtection.PlayerUse(ply, ent) == false) then return end

	if !ent.contents[res] then
		ply:SendMessage("The cache doesn't have any of that.", 60, Color(255, 255, 80, 255))
		return
	end
...
net.Receive( "cl_frompcache1", function( l, ply )
	...
	if !ent or !ent:GetClass() == "gms_pcache" then return end
	if !ent.POwner.pcontents then return end

	if(SPropProtection.PlayerUse(ply, ent) == false) then return end

	if !IsValid( ent.POwner ) then return end

	if !ent.POwner.pcontents[res] then
		ply:SendMessage("The cache doesn't have any of that.", 60, Color(255, 255, 80, 255))
		return
	end
	...
	if !( SPropProtection.Props[ent:EntIndex()].SteamID64 == ply:GetPlayerID() ) then
...
net.Receive( "cl_frompcache2", function( l, ply )
	...
	if !ent or !ent:GetClass() == "gms_pcache2" then return end
	if !ent.POwner.p2contents then return end

	if(SPropProtection.PlayerUse(ply, ent) == false) then return end

	if !IsValid( ent.POwner ) then return end

	if !ent.POwner.p2contents[res] then
		ply:SendMessage("The cache doesn't have any of that.", 60, Color(255, 255, 80, 255))
		return
	end
...
net.Receive( "cl_frompcache3", function( l, ply )
	...
	if !ent or !ent:GetClass() == "gms_pcache3" then return end
	if !ent.POwner.p3contents then return end

	if(SPropProtection.PlayerUse(ply, ent) == false) then return end

	if !IsValid( ent.POwner ) then return end

	if !ent.POwner.p3contents[res] then
		ply:SendMessage("The cache doesn't have any of that.", 60, Color(255, 255, 80, 255))
		return
	end
...
net.Receive( "cl_frompcache4", function( l, ply )
  ...
  if !ent or !ent:GetClass() == "gms_pcache4" then return end
	if !ent.POwner.p4contents then return end

  if(SPropProtection.PlayerUse(ply, ent) == false) then return end

  if !IsValid( ent.POwner ) then return end

	if !ent.POwner.p4contents[res] then
		ply:SendMessage("The cache doesn't have any of that.", 60, Color(255, 255, 80, 255))
		return
	end
...
net.Receive( "cl_frompcacheboss", function( l, ply )
	...
	if !ent or !ent:GetClass() == "gms_pcacheboss" then return end
	if !ent.POwner.pbosscontents then return end

	if(SPropProtection.PlayerUse(ply, ent) == false) then return end

	if !IsValid( ent.POwner ) then return end

	if !ent.POwner.pbosscontents[res] then
		ply:SendMessage("The cache doesn't have any of that.", 60, Color(255, 255, 80, 255))
		return
	end
...
net.Receive( "cl_frompcachemeteoric", function( l, ply )
	local res = net.ReadString()
	local amt = net.ReadInt( 16 )
	local ent = SGS_CacheValidCheck(ply, amt)

	if !ent or !ent:GetClass() == "gms_pcachemeteoric" then return end
	if !ent.POwner.pmeteoriccontents then return end

	if(SPropProtection.PlayerUse(ply, ent) == false) then return end

	if !IsValid( ent.POwner ) then return end

	if !ent.POwner.pmeteoriccontents[res] then
		ply:SendMessage("The cache doesn't have any of that.", 60, Color(255, 255, 80, 255))
		return
	end

	if ent.POwner.pmeteoriccontents[res] < amt then
		ply:SendMessage("Cache doesn't have that many, taking max.", 60, Color(255, 255, 80, 255))
		amt = ent.POwner.pmeteoriccontents[res]
	end
	...
	if !( SPropProtection.Props[ent:EntIndex()].SteamID64 == ply:GetPlayerID() ) then
		SGS.Log("** " .. ply:Nick() .. " (" .. ply:GetPlayerID() .. ") removed " .. amt .. "x of item " .. res .. " from the pcachemeteoric owned by: " .. SPropProtection.Props[ent:EntIndex()].Name .. " (" .. SPropProtection.Props[ent:EntIndex()].SteamID64 .. ")**" )
	end
...
net.Receive( "cl_fromtcache", function( l, ply )
	...
	if !(trace.Entity:GetClass() == "gms_tcache") then
		return
	end
...
net.Receive( "cl_fromptcache", function( l, ply )
	...
	if !(trace.Entity:GetClass() == "gms_ptcache") then
		return
	end
	...
	if !ent.POwner.ptcontents[ tool ] then
		ply:SendMessage("The cache doesn't have any: " .. CapAll(string.gsub(tool, "_", " ")) ..".", 60, Color(0, 255, 0, 255))
		return
	end
...
hook.Add( "DayLightChangeTime", "DayChange", function( time )
	...
	net.Broadcast()
		for k, v in pairs(player.GetAll()) do
			if !(v:GetInitialized() == INITSTATE_OK) then return end
			v:CheckLastDaily()
		end
	local today = os.date( "%A")
	local hoursuntilrestart = 21 - os.date( "%H")
	local minute = os.date( "%M")
	if today == "Saturday" or today == "Tuesday" then
		if hoursuntilrestart < 0 then return end
		if hoursuntilrestart == 1 then
			if printedmessage == false or printedmessage == nil then
				for k, v in pairs(player.GetAll()) do
				v:ChatPrint("Server restarts in " .. 60 - minute ..  " minutes!")
				end
				printedmessage = true
				timer.Simple( 900, function() printedmessage = false end )
			end
		end
	end
end)
...
function SGS_Bleed( ply, amt )
	if !IsValid(ply) then return end
	if !ply.bleedamt then ply.bleedamt = 0 end
...
function SGS_PropDamageHandling( ent, dmginfo )
	...
	if !GAMEMODE.Worlds:GetEntityWorldSpace( ent )then return end

	if GAMEMODE.Worlds:GetEntityWorldSpace( ent ).BreakableStructures then
		if !table.HasValue( GAMEMODE.Worlds:GetEntityWorldSpace( ent ).BreakableStructuresTbl, ent:GetClass() ) then return end
		if !ent.health and ent:GetClass() == "gms_prop" then SGS_CalcPropHealth( ent ) end
		if !ent.health then ent.health = 100 ent.maxhealth = 100 end
		ent.health = ent.health - amount
		if !ent.healthycolor then ent.healthycolor = ent:GetColor() end
...
function PlayerMeta:TogglePVP()
	...
	self:SendMessage("You must be at least survival level 5 to enable PvP!", 60, Color(255, 0, 0, 255))
	return end
...
if world.pvp then
	if ply:GetLevel("survival") < 5 then
	ply:SendMessage("You must be at least survival level 5 to enter PvP!", 60, Color(255, 0, 0, 255))
	local spawnarea = ents.FindByClass( "info_player_start" )
		local random_entry = math.random(#spawnarea)
		local newpos = spawnarea[random_entry]:GetPos()
...
function SGS_FlagPVPWorld( ply )
	if !IsValid(ply) then return end
	if !GAMEMODE.Worlds:GetEntityWorldSpace( ply ) then return end
	if !GAMEMODE.Worlds:GetEntityWorldSpace( ply ).pvp then return end
...
function SGS_SetAFK(ply)
	...
	if ply:GetGroundEntity() == NULL and !ply:InVehicle() then
...
function SGS_PlayerDisconnected( ply )
	...
	timer.Simple( 1, function()
		...
		for k, v in pairs(GAMEMODE.Worlds.tblWorlds) do
			if v.loaded and !v.occupied then
				SGS_ClearWorld( k )
			end
		end
	end )
...
function SGS_CheckBot( ply )
	if !ply.checkbottimethreshold then ply.checkbottimethreshold = 0 end
...
function PlayerMeta:UseElixir( potion )
	...
	if statboost == "speed" then
		if GAMEMODE.Tribes:GetTribeLevel( self ) >= 6 then
			self:SetWalkSpeed( 192 * statmod )
			self:SetRunSpeed( 288 * statmod  )
		else
			self:SetWalkSpeed( 160 * statmod )
			self:SetRunSpeed( 240 * statmod )
		end
	end
...
function SGS_StopElixirEffect( ply )
	...
	if !IsValid( ply ) then return end
	if !ply:IsConnected() then return end
...
function SGS_Entity_Find( ply, con, args)
	...
	if !GAMEMODE.getUser( args[1] ) then
		ply:SendMessage("No player found with that name.", 60, Color(255, 0, 0, 255))
	return end

	if args[2] == nil then
		ply:SendMessage("You need to give search radius", 60, Color(255, 0, 0, 255))
	return end
...
concommand.Add( "sgs_entity_find", SGS_Entity_Find)

function SGS_Entity_Bring( ply, con, args)
	...
	if args[2] == nil then
		ply:SendMessage("You need to give [ID] of an entity", 60, Color(255, 0, 0, 255))
	return end
	...
	Entity( args[2] ):SetPos( GAMEMODE.getUser( args[1] ):GetPos() )

end
concommand.Add( "sgs_entity_bring", SGS_Entity_Bring)
...
function GM:OnPlayerHitGround( ply, bInWater, bOnFloater, flFallSpeed )
	...
	if flFallSpeed >= 420 and flFallSpeed <= 1000 then
		...
		dmg = math.floor( 23 * dmg )
		local randomizer = math.random(-5, 5)
		dmg = dmg + randomizer
...
function PlayerMeta:UnbreakLeg( silent )
	...
	if GAMEMODE.Tribes:GetTribeLevel( self ) >= 6 then
		self:SetWalkSpeed( 192 )
		self:SetRunSpeed( 288 )
...
function SGS_ChatOpenRules( ply, text, _ )

    if (string.sub(string.lower(text), 1, 6) == "!rules") then
		ply:SendLua("SGS_OpenRules()")
		return false
    end

end
hook.Add( "PlayerSay", "SGS_ChatOpenRules", SGS_ChatOpenRules )


function SGS_ChatOpenSContent( ply, text, _ )

    if (string.sub(string.lower(text), 1, 8) == "!content") then
		ply:SendLua("SGS_OpenSContent()")
		return false
    end

end
hook.Add( "PlayerSay", "SGS_ChatOpenSContent", SGS_ChatOpenSContent )
...
--Custom ones
	...
	if ach == "welcome"  or ach == "all" then
		if !self:GetAch("welcome") then
			if self:GetLevel("survival") >= 1 then
				self:SetAch("welcome") 
			end
		end
	end
	
	if ach == "woahthisaintsobad"  or ach == "all" then
		if !self:GetAch("woahthisaintsobad") then
			if self:GetLevel("survival") >= 10 then
				self:SetAch("woahthisaintsobad") 
			end
		end
	end
	
	if ach == "cantstopyou"  or ach == "all" then
		if !self:GetAch("cantstopyou") then
			if self:GetLevel("survival") >= 20 then
				self:SetAch("cantstopyou") 
			end
		end
	end
	
	if ach == "almost"  or ach == "all" then
		if !self:GetAch("almost") then
			if self:GetLevel("survival") >= 30 then
			self:SetAch("almost") 
			end
		end
	end
	
	if ach == "didit"  or ach == "all" then
		if !self:GetAch("didit") then
			if self:GetLevel("survival") >= 40 then
				self:SetAch("didit")
	...
	if ach == "melon"  or ach == "all" then
        if !self:GetAch("melon") then
            if self:GetStat("general4") >= 1 then
               self:SetAch("melon")
                self:AddResource("the_cure", 1 )
            end
        end
    end
	
	if ach == "melon10"  or ach == "all" then
        if !self:GetAch("melon10") then
            if self:GetStat("general4") >= 10 then
               self:SetAch("melon10")
            end
        end
    end
	
	if ach == "nomelon"  or ach == "all" then
		if !self:GetAch("nomelon") then
			if self:GetStat("fishing1") >= 1 then
				self:SetAch("nomelon") 
			end
		end
	end
	
	if ach == "nomelon100"  or ach == "all" then
		if !self:GetAch("nomelon100") then
			if self:GetStat("fishing1") >= 100 then
				self:SetAch("nomelon100") 
			end
		end
	end
	
	if ach == "nomelon500"  or ach == "all" then
		if !self:GetAch("nomelon500") then
			if self:GetStat("fishing1") >= 500 then
				self:SetAch("nomelon500") 
			end
		end
	end
	
	if ach == "nomelon1000"  or ach == "all" then
		if !self:GetAch("nomelon1000") then
			if self:GetStat("fishing1") >= 1000 then
				self:SetAch("nomelon1000") 
			end
		end
	end
	
	if ach == "cute"  or ach == "all" then
		if !self:GetAch("cute") then
			if self:GetStat("general16") >= 1 then
				self:SetAch("cute") 
			end
		end
	end
	
	if ach == "cute10"  or ach == "all" then
		if !self:GetAch("cute10") then
			if self:GetStat("general16") >= 10 then
				self:SetAch("cute10") 
			end
		end
	end
	
	if ach == "cute100"  or ach == "all" then
		if !self:GetAch("cute100") then
			if self:GetStat("general16") >= 100 then
				self:SetAch("cute100") 
			end
		end
	end
	
	if ach == "cute500"  or ach == "all" then
		if !self:GetAch("cute500") then
			if self:GetStat("general16") >= 500 then
				self:SetAch("cute500")
				self:GiveGTokens( 1000 )				
			end
		end
	...
	if ach == "deadofage"  or ach == "all" then
		if !self:GetAch("deadofage") then
			if self:GetStat("pets11") >= 1 then
				self:SetAch("deadofage") 
				self:GiveGTokens( 50 )
			end
		end
	end
	
	if ach == "wizzy"  or ach == "all" then
		if !self:GetAch("wizzy") then
			if self:GetStat("arcane1") >= 1 then
				self:SetAch("wizzy")
				self:AddResource("wind_stone", 20 )
			end
		end
	end
	
	if ach == "wizzy100"  or ach == "all" then
		if !self:GetAch("wizzy100") then
			if self:GetStat("arcane1") >= 100 then
				self:SetAch("wizzy100")
			end
		end
	end
	
	if ach == "wizzy500"  or ach == "all" then
		if !self:GetAch("wizzy500") then
			if self:GetStat("arcane1") >= 500 then
				self:SetAch("wizzy500")
			end
		end
	end
	
	if ach == "wizzy1000"  or ach == "all" then
		if !self:GetAch("wizzy1000") then
			if self:GetStat("arcane1") >= 1000 then
				self:SetAch("wizzy1000")
			end
		end
	end
	
	if ach == "holdup"  or ach == "all" then
		if !self:GetAch("holdup") then
			if self:GetStat("construction1") >= 1000 then
				self:SetAch("holdup") 
				self:GiveGTokens( 100 )
			end
		end
	end
	
	if ach == "holdup2"  or ach == "all" then
		if !self:GetAch("holdup2") then
			if self:GetStat("construction1") >= 5000 then
				self:SetAch("holdup2") 
			end
		end
	end
	
	if ach == "noholdup"  or ach == "all" then
		if !self:GetAch("noholdup") then
			if self:GetStat("construction1") >= 1000 then
				self:SetAch("noholdup") 
			end
		end
	end
	
	if ach == "noholdup2"  or ach == "all" then
		if !self:GetAch("noholdup2") then
			if self:GetStat("construction1") >= 5000 then
				self:SetAch("noholdup2") 
				self:GiveGTokens( 500 )
			end
		end
	end
	
	if ach == "missclick"  or ach == "all" then
		if !self:GetAch("missclick") then
			if self:GetStat("combat5") >= 1 then
				self:SetAch("missclick") 
			end
		end
	end
	
	if ach == "missclick5"  or ach == "all" then
		if !self:GetAch("missclick5") then
			if self:GetStat("combat5") >= 5 then
				self:SetAch("missclick5") 
			end
		end
	end
	
	if ach == "missclick50"  or ach == "all" then
		if !self:GetAch("missclick50") then
			if self:GetStat("combat5") >= 50 then
				self:SetAch("missclick50")
				self:GiveGTokens( 250 )				
			end
		end
	end
	
	if ach == "thirsty10"  or ach == "all" then
		if !self:GetAch("thirsty10") then
			if self:GetStat("general1") >= 10 then
				self:SetAch("thirsty10") 
			end
		end
	end
	
	if ach == "berries10"  or ach == "all" then
		if !self:GetAch("berries10") then
			if self:GetStat("general2") >= 10 then
				self:SetAch("berries10") 
			end
		end
	end
	
	if ach == "berries100"  or ach == "all" then
		if !self:GetAch("berries100") then
			if self:GetStat("general2") >= 100 then
				self:SetAch("berries100") 
			end
		end
	end
	
	if ach == "berries1000"  or ach == "all" then
		if !self:GetAch("berries1000") then
			if self:GetStat("general2") >= 1000 then
				self:SetAch("berries1000")
				self:GiveGTokens( 250 )
			end
		end
	end

	if ach == "sleepy10"  or ach == "all" then
		if !self:GetAch("sleepy10") then
			if self:GetStat("general3") >= 10 then
				self:SetAch("sleepy10") 
			end
		end
	end
	
	if ach == "sleepy100"  or ach == "all" then
		if !self:GetAch("sleepy100") then
			if self:GetStat("general3") >= 100 then
				self:SetAch("sleepy100") 
			end
		end
	end
	
	if ach == "sleepy1000"  or ach == "all" then
		if !self:GetAch("sleepy1000") then
			if self:GetStat("general3") >= 1000 then
				self:SetAch("sleepy1000") 
				self:GiveGTokens( 500 )
			end
		end
	end
	
	if ach == "die1"  or ach == "all" then
		if !self:GetAch("die1") then
			if self:GetStat("general5") >= 1 then
				self:SetAch("die1") 
			end
		end
	end
	
	if ach == "die10"  or ach == "all" then
		if !self:GetAch("die10") then
			if self:GetStat("general5") >= 10 then
				self:SetAch("die10") 
			end
		end
	end
	
	if ach == "die100"  or ach == "all" then
		if !self:GetAch("die100") then
			if self:GetStat("general5") >= 100 then
				self:SetAch("die100") 
			end
		end
	end
	
	if ach == "die500"  or ach == "all" then
		if !self:GetAch("die500") then
			if self:GetStat("general5") >= 500 then
				self:SetAch("die500")
				self:GiveGTokens( 500 )
			end
		end
	...
	if ach == "relic1"  or ach == "all" then
		if !self:GetAch("relic1") then
			if self:GetStat("general11") >= 1 then
				self:SetAch("relic1") 
			end
		end
	end
	
	if ach == "relic10"  or ach == "all" then
		if !self:GetAch("relic10") then
			if self:GetStat("general11") >= 10 then
				self:SetAch("relic10") 
			end
		end
	end
	
	if ach == "relic50"  or ach == "all" then
		if !self:GetAch("relic50") then
			if self:GetStat("general11") >= 50 then
				self:SetAch("relic50") 
				self:GiveGTokens( 500 )
			end
		end
	...
	if ach == "artifact1"  or ach == "all" then
		if !self:GetAch("artifact1") then
			if self:GetStat("general12") >= 1 then
				self:SetAch("artifact1") 
			end
		end
	end
	
	if ach == "artifact5"  or ach == "all" then
		if !self:GetAch("artifact5") then
			if self:GetStat("general12") >= 5 then
				self:SetAch("artifact5") 
			end
		end
	end
	
	if ach == "artifact50"  or ach == "all" then
		if !self:GetAch("artifact50") then
			if self:GetStat("general12") >= 50 then
				self:SetAch("artifact50")
				self:GiveGTokens( 250 )
			end
		end
	end
	
	if ach == "gtoken1"  or ach == "all" then
		if !self:GetAch("gtoken1") then
			if self:GetStat("general13") >= 1 then
				self:SetAch("gtoken1") 
			end
		end
	end
	
	if ach == "gtoken750"  or ach == "all" then
		if !self:GetAch("gtoken750") then
			if self:GetStat("general13") >= 750 then
				self:SetAch("gtoken750") 
			end
		end
	end
	
	if ach == "sgtoken50000"  or ach == "all" then
		if !self:GetAch("sgtoken50000") then
			if self:GetStat("general14") >= 50000 then
				self:SetAch("sgtoken50000") 
			end
		end
	end
	
	if ach == "woodc100"  or ach == "all" then
		if !self:GetAch("woodc100") then
			if self:GetStat("woodcutting1") >= 100 then
				self:SetAch("woodc100") 
			end
		end
	end
	
	if ach == "woodc500"  or ach == "all" then
		if !self:GetAch("woodc500") then
			if self:GetStat("woodcutting1") >= 500 then
				self:SetAch("woodc500") 
			end
		end
	end
	
	if ach == "woodc1000"  or ach == "all" then
		if !self:GetAch("woodc1000") then
			if self:GetStat("woodcutting1") >= 1000 then
				self:SetAch("woodc1000") 
			end
		end
	end
	
	if ach == "woodc5000"  or ach == "all" then
		if !self:GetAch("woodc5000") then
			if self:GetStat("woodcutting1") >= 5000 then
				self:SetAch("woodc5000")
				self:GiveGTokens( 750 )
			end
		end
	end
	
	if ach == "woodf10"  or ach == "all" then
		if !self:GetAch("woodf10") then
			if self:GetStat("woodcutting2") >= 10 then
				self:SetAch("woodf10") 
			end
		end
	end
	
	if ach == "woodf100"  or ach == "all" then
		if !self:GetAch("woodf100") then
			if self:GetStat("woodcutting2") >= 100 then
				self:SetAch("woodf100") 
			end
		end
	end

	if ach == "woodf1000"  or ach == "all" then
		if !self:GetAch("woodf1000") then
			if self:GetStat("woodcutting2") >= 1000 then
				self:SetAch("woodf1000") 
			end
		end
	end
	
	if ach == "woodf5000"  or ach == "all" then
		if !self:GetAch("woodf5000") then
			if self:GetStat("woodcutting2") >= 5000 then
				self:SetAch("woodf5000") 
			end
		end
	end
	
	if ach == "woode1"  or ach == "all" then
		if !self:GetAch("woode1") then
			if self:GetStat("woodcutting3") >= 1 then
				self:SetAch("woode1") 
			end
		end
	end
	
	if ach == "woode100"  or ach == "all" then
		if !self:GetAch("woode100") then
			if self:GetStat("woodcutting3") >= 100 then
				self:SetAch("woode100") 
				self:AddResource( "enchanted_wood", 5 )
			end
		end
	end
	
	if ach == "woode5"  or ach == "all" then
		if !self:GetAch("woode5") then
			if self:GetStat("woodcutting3") >= 5 then
				self:SetAch("woode5")
				self:AddResource( "enchanted_wood", 1 )
			end
		end
	end
	
	if ach == "woode50"  or ach == "all" then
		if !self:GetAch("woode50") then
			if self:GetStat("woodcutting3") >= 50 then
				self:SetAch("woode50") 
			end
		end
	end
	
	if ach == "ored10"  or ach == "all" then
		if !self:GetAch("ored10") then
			if self:GetStat("mining7") >= 10 then
				self:SetAch("ored10") 
			end
		end
	end
	
	if ach == "ored100"  or ach == "all" then
		if !self:GetAch("ored100") then
			if self:GetStat("mining7") >= 100 then
				self:SetAch("ored100") 
			end
		end
	end
	
	if ach == "ored1000"  or ach == "all" then
		if !self:GetAch("ored1000") then
			if self:GetStat("mining7") >= 1000 then
				self:SetAch("ored1000") 
			end
		end
	end
	
	if ach == "ored5000"  or ach == "all" then
		if !self:GetAch("ored5000") then
			if self:GetStat("mining7") >= 5000 then
				self:SetAch("ored5000") 
			end
		end
	end
	
	if ach == "gems1"  or ach == "all" then
		if !self:GetAch("gems1") then
			if self:GetStat("mining8") >= 1 then
				self:SetAch("gems1") 
			end
		end
	end
	
	if ach == "gems10"  or ach == "all" then
		if !self:GetAch("gems10") then
			if self:GetStat("mining8") >= 10 then
				self:SetAch("gems10") 
			end
		end
	end
	
	if ach == "gems50"  or ach == "all" then
		if !self:GetAch("gems50") then
			if self:GetStat("mining8") >= 50 then
				self:SetAch("gems50") 
			end
		end
	end
	
	if ach == "gems100"  or ach == "all" then
		if !self:GetAch("gems100") then
			if self:GetStat("mining8") >= 100 then
				self:SetAch("gems100") 
			end
		end
	end
	
	if ach == "meteorc1"  or ach == "all" then
		if !self:GetAch("meteorc1") then
			if self:GetStat("mining13") >= 1 then
				self:SetAch("meteorc1") 
			end
		end
	end
	
	if ach == "meteorc10"  or ach == "all" then
		if !self:GetAch("meteorc10") then
			if self:GetStat("mining13") >= 10 then
				self:SetAch("meteorc10") 
			end
		end
	end
	
	if ach == "meteorc50"  or ach == "all" then
		if !self:GetAch("meteorc50") then
			if self:GetStat("mining13") >= 50 then
				self:SetAch("meteorc50")
				self:AddResource( "meteoric_iron_ore", 3 )
			end
		end
	end
	
	if ach == "plants50"  or ach == "all" then
		if !self:GetAch("plants50") then
			if self:GetStat("farming1") >= 50 then
				self:SetAch("plants50") 
			end
		end
	end
	
	if ach == "plants500"  or ach == "all" then
		if !self:GetAch("plants500") then
			if self:GetStat("farming1") >= 500 then
				self:SetAch("plants500") 
			end
		end
	end
	
	if ach == "plants5000"  or ach == "all" then
		if !self:GetAch("plants5000") then
			if self:GetStat("farming1") >= 5000 then
				self:SetAch("plants5000") 
			end
		end
	end
	
	if ach == "forage50"  or ach == "all" then
		if !self:GetAch("forage50") then
			if self:GetStat("farming2") >= 50 then
				self:SetAch("forage50") 
			end
		end
	end
	
	if ach == "forage500"  or ach == "all" then
		if !self:GetAch("forage500") then
			if self:GetStat("farming2") >= 500 then
				self:SetAch("forage500") 
			end
		end
	end
	
	if ach == "forage5000"  or ach == "all" then
		if !self:GetAch("forage5000") then
			if self:GetStat("farming2") >= 5000 then
				self:SetAch("forage5000") 
			end
		end
	end
	
	if ach == "forage10000"  or ach == "all" then
		if !self:GetAch("forage10000") then
			if self:GetStat("farming2") >= 10000 then
				self:SetAch("forage10000")
				self:GiveGTokens( 750 )
			end
		end
	end
	
	if ach == "goldm1"  or ach == "all" then
		if !self:GetAch("goldm1") then
			if self:GetStat("farming3") >= 1 then
				self:SetAch("goldm1") 
			end
		end
	end
	
	if ach == "goldm10"  or ach == "all" then
		if !self:GetAch("goldm10") then
			if self:GetStat("farming3") >= 10 then
				self:SetAch("goldm10")
				self:GiveGTokens( 250 )
			end
		end
	...
	if ach == "losefish10"  or ach == "all" then
		if !self:GetAch("losefish10") then
			if self:GetStat("fishing2") >= 10 then
				self:SetAch("losefish10") 
			end
		end
	end
	
	if ach == "losefish100"  or ach == "all" then
		if !self:GetAch("losefish100") then
			if self:GetStat("fishing2") >= 100 then
				self:SetAch("losefish100") 
			end
		end
	end
	
	if ach == "losefish1000"  or ach == "all" then
		if !self:GetAch("losefish1000") then
			if self:GetStat("fishing2") >= 1000 then
				self:SetAch("losefish1000")
				self:GiveGTokens( 250 )				
			end
		end
	end
	
	if ach == "craft1"  or ach == "all" then
		if !self:GetAch("craft1") then
			if self:GetStat("smithing1") >= 1 then
				self:SetAch("craft1") 
			end
		end
	end
	
	if ach == "craft10"  or ach == "all" then
		if !self:GetAch("craft10") then
			if self:GetStat("smithing1") >= 10 then
				self:SetAch("craft10") 
			end
		end
	end
	
	if ach == "craft100"  or ach == "all" then
		if !self:GetAch("craft100") then
			if self:GetStat("smithing1") >= 100 then
				self:SetAch("craft100") 
			end
		end
	end
	
	if ach == "craft1000"  or ach == "all" then
		if !self:GetAch("craft1000") then
			if self:GetStat("smithing1") >= 1000 then
				self:SetAch("craft1000")
				self:GiveGTokens( 1000 )				
			end
		end
	end
	
	if ach == "cookery10"  or ach == "all" then
		if !self:GetAch("cookery10") then
			if self:GetStat("cooking9") >= 10 then
				self:SetAch("cookery10") 
			end
		end
	end
	
	if ach == "cookery100"  or ach == "all" then
		if !self:GetAch("cookery100") then
			if self:GetStat("cooking9") >= 100 then
				self:SetAch("cookery100") 
			end
		end
	end
	
	if ach == "cookery1000"  or ach == "all" then
		if !self:GetAch("cookery1000") then
			if self:GetStat("cooking9") >= 1000 then
				self:SetAch("cookery1000") 
			end
		end
	end
	
	if ach == "structurem5"  or ach == "all" then
		if !self:GetAch("structurem5") then
			if self:GetStat("construction3") >= 5 then
				self:SetAch("structurem5") 
			end
		end
	end
	
	if ach == "structurem50"  or ach == "all" then
		if !self:GetAch("structurem50") then
			if self:GetStat("construction3") >= 50 then
				self:SetAch("structurem50") 
			end
		end
	end
	
	if ach == "structurem500"  or ach == "all" then
		if !self:GetAch("structurem500") then
			if self:GetStat("construction3") >= 500 then
				self:SetAch("structurem500") 
				self:GiveGTokens( 500 )
			end
		end
	end
	
	if ach == "antlion10"  or ach == "all" then
		if !self:GetAch("antlion10") then
			if self:GetStat("combat1") >= 10 then
				self:SetAch("antlion10") 
			end
		end
	end
	
	if ach == "antlion100"  or ach == "all" then
		if !self:GetAch("antlion100") then
			if self:GetStat("combat1") >= 100 then
				self:SetAch("antlion100") 
			end
		end
	end
	
	if ach == "antlion1000"  or ach == "all" then
		if !self:GetAch("antlion1000") then
			if self:GetStat("combat1") >= 1000 then
				self:SetAch("antlion1000")
				self:GiveGTokens( 500 )				
			end
		end
	end
	
	if ach == "dieinpvp1"  or ach == "all" then
		if !self:GetAch("dieinpvp1") then
			if self:GetStat("combat6") >= 1 then
				self:SetAch("dieinpvp1") 
			end
		end
	end
	
	if ach == "dieinpvp5"  or ach == "all" then
		if !self:GetAch("dieinpvp5") then
			if self:GetStat("combat6") >= 5 then
				self:SetAch("dieinpvp5") 
			end
		end
	end
	
	if ach == "dieinpvp25"  or ach == "all" then
		if !self:GetAch("dieinpvp25") then
			if self:GetStat("combat6") >= 25 then
				self:SetAch("dieinpvp25")
				self:GiveGTokens( 250 )				
			end
		end
	end
	
	if ach == "pbrew1"  or ach == "all" then
		if !self:GetAch("pbrew1") then
			if self:GetStat("alchemy1") >= 1 then
				self:SetAch("pbrew1") 
			end
		end
	end
	
	if ach == "pbrew50"  or ach == "all" then
		if !self:GetAch("pbrew50") then
			if self:GetStat("alchemy1") >= 50 then
				self:SetAch("pbrew50") 
			end
		end
	end
	
	if ach == "pbrew500"  or ach == "all" then
		if !self:GetAch("pbrew500") then
			if self:GetStat("alchemy1") >= 500 then
				self:SetAch("pbrew500")
				self:GiveGTokens( 250 )				
			end
		end
	end
	
	if ach == "pdrink5"  or ach == "all" then
		if !self:GetAch("pdrink5") then
			if self:GetStat("alchemy2") >= 5 then
				self:SetAch("pdrink5") 
			end
		end
	end
	
	if ach == "pdrink50"  or ach == "all" then
		if !self:GetAch("pdrink50") then
			if self:GetStat("alchemy2") >= 50 then
				self:SetAch("pdrink50") 
			end
		end
	end
	
	if ach == "pdrink500"  or ach == "all" then
		if !self:GetAch("pdrink500") then
			if self:GetStat("alchemy2") >= 500 then
				self:SetAch("pdrink500")
				self:GiveGTokens( 250 )				
			end
		end
	end
	
	if ach == "transmute100"  or ach == "all" then
		if !self:GetAch("transmute100") then
			if self:GetStat("alchemy4") >= 100 then
				self:SetAch("transmute100") 
			end
		end
	end
	
	if ach == "transmute5000"  or ach == "all" then
		if !self:GetAch("transmute5000") then
			if self:GetStat("alchemy4") >= 5000 then
				self:SetAch("transmute5000") 
			end
		end
	end
	
	if ach == "tp1"  or ach == "all" then
		if !self:GetAch("tp1") then
			if self:GetStat("arcane2") >= 1 then
				self:SetAch("tp1") 
			end
		end
	end
	
	if ach == "tp50"  or ach == "all" then
		if !self:GetAch("tp50") then
			if self:GetStat("arcane2") >= 50 then
				self:SetAch("tp50") 
			end
		end
	end
	
	if ach == "tp500"  or ach == "all" then
		if !self:GetAch("tp500") then
			if self:GetStat("arcane2") >= 500 then
				self:SetAch("tp500")
				self:GiveGTokens( 500 )				
			end
		end
	...
	if ach == "bloodzombie10"  or ach == "all" then
		if !self:GetAch("bloodzombie10") then
			if self:GetStat("bloodmoon2") >= 10 then
				self:SetAch("bloodzombie10") 
			end
		end
	end
	
	if ach == "bloodzombie100"  or ach == "all" then
		if !self:GetAch("bloodzombie100") then
			if self:GetStat("bloodmoon2") >= 100 then
				self:SetAch("bloodzombie100") 
			end
		end
	end
	
	if ach == "bloodzombie1000"  or ach == "all" then
		if !self:GetAch("bloodzombie1000") then
			if self:GetStat("bloodmoon2") >= 1000 then
				self:SetAch("bloodzombie1000")
				self:GiveGTokens( 1000 )
			end
		end
	end
	
	if ach == "afk10"  or ach == "all" then
		if !self:GetAch("afk10") then
			if self:GetStat("time2") >= 10 then
				self:SetAch("afk10") 
			end
		end
	end
	
	if ach == "afk60"  or ach == "all" then
		if !self:GetAch("afk60") then
			if self:GetStat("time2") >= 60 then
				self:SetAch("afk60") 
			end
		end
	end
	
	if ach == "afk1440"  or ach == "all" then
		if !self:GetAch("afk1440") then
			if self:GetStat("time2") >= 1440 then
				self:SetAch("afk1440") 
			end
		end
	end
	
	if ach == "burn1"  or ach == "all" then
		if !self:GetAch("burn1") then
			if self:GetStat("cooking5") >= 1 then
				self:SetAch("burn1") 
			end
		end
	end
	
	if ach == "burn100"  or ach == "all" then
		if !self:GetAch("burn100") then
			if self:GetStat("cooking5") >= 100 then
				self:SetAch("burn100") 
			end
		end
	end
	
	if ach == "burn1000"  or ach == "all" then
		if !self:GetAch("burn1000") then
			if self:GetStat("cooking5") >= 1000 then
				self:SetAch("burn1000") 
			end
		end
	end
	
	if ach == "burn5000"  or ach == "all" then
		if !self:GetAch("burn5000") then
			if self:GetStat("cooking5") >= 5000 then
				self:SetAch("burn5000")
				self:GiveGTokens( 1000 )				
			end
		end
	end
...
--End of custom ones
	...
	if ach == "voidcache1" or ach == "all" then
		if !self:GetAch("voidcache1") then
			if self:GetStat("general17") >= 100 then
				self:SetAch("voidcache1")
				self:AddResource( "void_cache", 5 )
			end
		end
	end
	if ach == "voidcache2" or ach == "all" then
		if !self:GetAch("voidcache2") then
			if self:GetStat("general18") >= 50 then
				self:SetAch("voidcache2")
				self:AddResource( "void_crystal", 3 )
			end
		end
	end
	
	if ach == "voidcache10" or ach == "all" then
		if !self:GetAch("voidcache10") then
			if self:GetStat("general17") >= 10 then
				self:SetAch("voidcache10")
				self:AddResource( "void_cache", 1 )
			end
		end
	end
	
	if ach == "voidcacheo10" or ach == "all" then
		if !self:GetAch("voidcacheo10") then
			if self:GetStat("general18") >= 10 then
				self:SetAch("voidcacheo10")
				self:AddResource( "void_crystal", 1 )
			end
		end
	end
	
	if ach == "zombieslayer" or ach == "all" then
		if !self:GetAch("zombieslayer") then
	...
	if ach == "zombieslayer2" or ach == "all" then
		if !self:GetAch("zombieslayer2") then
			if self:GetStat("combat8") >= 10 then
				self:SetAch("zombieslayer2")
			end
		end
	end
	
	if ach == "zombieslayer3" or ach == "all" then
		if !self:GetAch("zombieslayer3") then
			if self:GetStat("combat8") >= 100 then
				self:SetAch("zombieslayer3")
			end
		end
	end
	
	if ach == "bloodchampion" or ach == "all" then
		if !self:GetAch("bloodchampion") then
	...
	if ach == "bloodchampion1" or ach == "all" then
		if !self:GetAch("bloodchampion1") then
			if self:GetStat("bloodmoon1") >= 1 then
				self:SetAch("bloodchampion1")
			end
		end
	end
	
	if ach == "bloodchampion2" or ach == "all" then
		if !self:GetAch("bloodchampion2") then
			if self:GetStat("bloodmoon1") >= 50 then
				self:SetAch("bloodchampion2")
			end
		end
	end
	
	if ach == "fishmaster" or ach == "all" then
		if !self:GetAch("fishmaster") then
	...
	if ach == "woodcuttingmaster" or ach == "all" then
		if !self:GetAch("woodcuttingmaster") then
	...
	if ach == "miningmaster" or ach == "all" then
		if !self:GetAch("miningmaster") then
	...
	if ach == "constructionmaster" or ach == "all" then
		if !self:GetAch("constructionmaster") then
	...
	if ach == "farmingmaster" or ach == "all" then
		if !self:GetAch("farmingmaster") then
	...
	if ach == "smithingmaster" or ach == "all" then
		if !self:GetAch("smithingmaster") then
	...
	if ach == "arcanemaster" or ach == "all" then
		if !self:GetAch("arcanemaster") then
	...
	if ach == "alchemymaster" or ach == "all" then
		if !self:GetAch("alchemymaster") then
	...
	if ach == "cookingmaster" or ach == "all" then
		if !self:GetAch("cookingmaster") then
	...
	if ach == "farminghat" or ach == "all" then
		if !self:GetAch("farminghat") then
	...
	if ach == "wizardhat" or ach == "all" then
		if !self:GetAch("wizardhat") then
			if self:GetStat("arcane3") >= 2500 then
				self:SetAch("wizardhat")
			end
		end
	end
	if ach == "killaboss" or ach == "all" then
		if !self:GetAch("killaboss") then
			if self:GetStat("combat3") >= 1 then
				self:SetAch("killaboss")
			end
		end
	end
	
	if ach == "killaboss2" or ach == "all" then
		if !self:GetAch("killaboss2") then
			if self:GetStat("combat3") >= 10 then
				self:SetAch("killaboss2")
			end
		end
	end
	
	if ach == "killaboss3" or ach == "all" then
		if !self:GetAch("killaboss3") then
			if self:GetStat("combat3") >= 50 then
				self:SetAch("killaboss3")
				self:AddResource( "btoken", 25 )
			end
		end
	end
	
	if ach == "goggleshat" or ach == "all" then
		if !self:GetAch("goggleshat") then
	...
	if ach == "squirrelhat" or ach == "all" then
		if !self:GetAch("squirrelhat") then
	...
	if ach == "headcrabhat" or ach == "all" then
		if !self:GetAch("headcrabhat") then
			...
			if self:GetStat("time1") >= 9000 then
				...
				if !self:IsDonator() then
					if self:GetAch("time5") and ( self:IsUserGroup( "member" ) or self:IsUserGroup( "user" ) ) then
						RunConsoleCommand("ulx", "adduserid", self:SteamID(), "veteran")
						SGS_SetUpTeams(self)
					end
				end
			end
...
function PlayerMeta:SetSettings()
	...
	local chcolor = self:GetSetting("crosshaircolor")
	...
	if chcolor then
		self:ConCommand( "sgs_getcrosshaircolor" )
	end
...
function SGS_ConViewOthersStats( ply, com, args )
	...
	if !other.stats then return end
...
function SGS_ConSetCrosshairColor( ply, _, args )
	...
	ply:SetSetting( "crosshaircolor", math.Clamp( chcolor.r, 0, 255 ), math.Clamp( chcolor.g, 0, 255 ), math.Clamp( chcolor.b, 0, 255 ), math.Clamp( chcolor.a, 0, 255 ) )

end
concommand.Add("sgs_setcrosshaircolor", SGS_ConSetCrosshairColor)
...
function SGS_ConBuyHat( ply, com, args )
		...
		if !SGS_ReverseHatLookup( args[1] ) then
			ply:SendMessage("Invalid item!", 60, Color(0, 255, 0, 255))
			return
		end
...
function SGS_MarkForDeletionOwnerless( ent )
	...
	if !IsValid( ent ) then return end
	if !ent.ownerless then return end
...
function PlayerMeta:TimeCountUp()
    ...
    if self.tosaccept or self.spawnloc != self:GetPos() then
        if !self.sessiontime then self.sessiontime = 0 end
        if !self.afktime then self.afktime = 0 end
...
function SGS_SpawnMeteor()
	...
	RunConsoleCommand( "ulx", "csay", "A meteor has spawned!" )
	for k, v in pairs( player.GetAll() ) do
...
hook.Add( "Think", "SpawnMeteorCheck", function()
	if !SGS.lastmeteorcheck then SGS.lastmeteorcheck = CurTime() end
...
function MeteorCheck()
	if !SGS.nextmeteor then SGS.nextmeteor = 0 end
...
function GM:PlayerShouldTaunt( ply )
	if !ply.nexttaunt then ply.nexttaunt = CurTime() - 100 end
...
--		ply:SendLua("gui.OpenURL('http://steamcommunity.com/groups/projectgullG4P/')")
--		return false
--	end
...
function PlayerMeta:Resurrect()
	...
	if !IsValid( self.m_hRagdollEntity ) then return end
	if !self.res_sickness then self.res_sickness = 0 end
...
function GM:PlayerDeathThink( ply )
	ply.ressable = true
	if ( math.floor( CurTime() ) - ply:GetNWInt("lastdeathtime") >= ply:GetNWInt("deathtotaltime", 60) ) and !ply:GetNWBool("displaydeathnotice", false) then
		ply:DeathTwo()
	end
	return false
end
