...
function ENT:SetResourceDropInfo( rType, rAmt )
	...
	self.POwner.pmeteoriccontents[rType] = (self.POwner.pmeteoriccontents[rType] or 0) + rAmt
...
function ENT:GetTotalResources()
	...
	local total = 0
	for k, v in pairs( self.POwner.pmeteoriccontents ) do
		if v<0 then v=0 end
		total = total + v
	end
	return total
...
function ENT:Use( ply )
	...
	net.Start("UpdateCacheTable")
		net.WriteTable( self.POwner.pmeteoriccontents )
	net.Send( ply )
...
function ENT:CacheEnable( ply )
	self:SetColor(Color(255, 70, 0,255))
	
	self.enabled = true
	if ply then 
		self.POwner = ply
		if ply.pmeteoriccontents == nil then
			ply.pmeteoriccontents = {}
		end
	end
end
