...
function ENT:StartTouchPlayersNPCs(e,ignore)
	...
	if(e:IsPlayerHolding()) then
		...
		if(not self:IsOpen() or self.ShuttingDownKill) then ply:Kill() end; -- We havent opened yet. Kill people!
		...
		if(not block and not IsValid(self.Target)) then block = false end; -- Do we have a valid target? If not, we are inbound
...
function ENT:EndTouch(e)
	...
	if(e:IsPlayerHolding()) then
		...
		local block = false;
		local target_gate; -- Used in here and a bit lower later (for the iris hit sounds)
		if(not self:IsOpen()) then ply:Kill() end; -- We havent opened yet. Kill people!
		-- Are we entering the eventhorizon from the wrong side? Kill us!
		local dir = (self.Entity:GetVelocity()-e:GetVelocity()):GetNormalized();
