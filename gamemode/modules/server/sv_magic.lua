...
function PlayerMeta:CastSpell( spell )
	...
	if !SGS.Spells[spell] then return end
...
SPELL.func = function( self )
	...
	if self:GetNWBool("hasquest") == true and self:GetNWString("Quest") == "Cast spells." then
				self:SetNWInt("amounttodo", self:GetNWInt("amounttodo") - 1 )
				if self:GetNWInt("amounttodo") == 0 then
					self:RemoveQuest()
					self:AddResource("quest_token", 1)
					if self:GetNWBool("haswquest") == true and self:GetNWString("Weekly") == "Complete Dailies." then
							self:SetNWInt("wamounttodo", self:GetNWInt("wamounttodo") - 1 )
							if self:GetNWInt("wamounttodo") == 0 then
...
SPELL.func = function( self )
	...
	if !IsValid( self ) then return end
	if ( self.lastuse + 1200 > CurTime() ) and ( !self:IsSuperAdmin() ) then
		self:SendMessage("You must wait " .. math.floor(self.lastuse + 1200 - CurTime()) .. " more seconds before summoning a bloodmoon.", 60, Color(255, 0, 0, 255))
		self.failedspell = true
		return false
	end
	
	if GAMEMODE.Worlds.BloodMoon then
		self:SendMessage( "You cannot summon a bloodmoon while there already is one.", 60, Color( 255, 0, 0, 255 ) )
		return false
	else
		GAMEMODE.Worlds.Apocalypse = true
		self:SendMessage( "Bloodmoon will spawn the next night", 60, Color( 255, 0, 0, 255 ) )
...
SPELL.func = function( self )
	if !IsValid( self ) then return end
	if ( self.timechange + 600 > CurTime() ) and ( !self:IsSuperAdmin() ) then
...
SPELL.func = function( self )
	...
	if !self.lastcasthealself then self.lastcasthealself = CurTime() - 15 end
...
SPELL.func = function( self )
	...
	if !self.lastcastfixself then self.lastcastfixself = CurTime() - 15 end
	
	
	if self.lastcastfixself + 10 > CurTime() then
		self:SendMessage("You must wait " .. math.floor(self.lastcastfixself + 10 - CurTime()) .. " more seconds before fixing your leg again.", 60, Color(255, 0, 0, 255))
		self.failedspell = true
		return false
	end
	...
	if self.brokenleg then
		self:UnbreakLeg()
		self:SendMessage("Leg fixed!", 60, Color(0, 255, 0, 255))
	end
...
SPELL.func = function( self )
	...
	if tarply.brokenleg then
	tarply:UnbreakLeg()
	tarply:SendMessage("Leg fixed by " .. self:Nick(), 60, Color(0, 255, 0, 255))
	end
...
function SGS_SpellCastCon( ply, com, args )
	...
	if !#args == 1 then return end
	if !SGS_ReverseSpellLookup( args[1] ) then return end
