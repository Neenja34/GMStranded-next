...
function SGS_SetZombiesNeutral( ply )
	...
	for k, v in pairs( ents.FindByClass("npc_zombie") ) do
		if IsValid(v) and v:IsNPC() and (!v.ispet) then
...
function SGS_NPCDamage( ent, dmginfo )
	...
	if !IsValid( infl ) then return end
	...
	if ent:IsNPC() or ent:IsNextBot() then
		...
		elseif infl:GetClass() == "grenade_spit" then
			if !ent:GetClass() == "npc_antlionguard" then return end
...
function SGS_NPCKilled( npc, ply, wep )
	...
	if SGS_ReverseNPCLookup(npc:GetClass()) then
		...
		for k, v in pairs( ptable ) do
			...
			if !v:IsConnected() then continue end
			if !GAMEMODE.Worlds:GetEntityWorldSpace( npc ) == GAMEMODE.Worlds:GetEntityWorldSpace( v ) then continue end
			v:AddExp( "combat", exp )
			if npc:GetClass() == "npc_antlion" then
			v:AddStat( "combat1", 1 )
			if v:GetNWBool("hasquest") == true and v:GetNWString("Quest") == "Kill Creatures." then
				ply:SetNWInt("amounttodo", ply:GetNWInt("amounttodo") - 1 )
				if ply:GetNWInt("amounttodo") == 0 then
					ply:RemoveQuest()
					ply:AddResource("quest_token", 1)
						if v:GetNWBool("haswquest") == true and v:GetNWString("Weekly") == "Complete Dailies." then
			...
			if npc:GetClass() == "npc_antlionguard" then v:AddStat( "combat3", 1 ) v:CheckForAchievements( "killaboss" ) end
			if npc:GetClass() == "npc_hunter" then v:AddStat( "combat3", 1 ) v:CheckForAchievements( "killaboss" ) end
			if npc:GetClass() == "npc_zombie" then 
			v:AddStat( "combat8", 1 ) 
			v:CheckForAchievements( "zombieslayer" ) 
			if v:GetNWBool("hasquest") == true and v:GetNWString("Quest") == "Kill Creatures." then
				ply:SetNWInt("amounttodo", ply:GetNWInt("amounttodo") - 1 )
				if ply:GetNWInt("amounttodo") == 0 then
					ply:RemoveQuest()
...
function SGS_NPCLoot( ltype, npc, ply )
	...
	elseif ltype == "antlionboss" then
		...
		if math.random(1,1) == 1 then
			ent:SetResourceDropInfo( "btoken", 2 )
		end
		
		if math.random(1,52) == 1 then
		...
		if raredrop == false then
			...
			if chance == 1 then
				ent:SetResourceDropInfo( "pink_pet_egg", 1 )
			elseif chance > 1 and chance <= 2 then
				ent:SetResourceDropInfo( "boss_pick_head", 1 )
			elseif chance > 4 and chance <= 6 then
				ent:SetResourceDropInfo( "boss_hatchet_head", 1 )
			elseif chance > 7 and chance <= 8 then
				ent:SetResourceDropInfo( "boss_rod_core", 1 )
...
function SGS_SpawnNPC( calling_ply, classname )
	if !calling_ply:IsValid() then return end
...
function SGS_SpawnZombieNightTime()
	...
	if SGS_DayPhase() == "night" then
		...
		for k, v in pairs( player.GetAll() ) do
			...
			if !GAMEMODE.Worlds:GetWorld( v ) then continue end
			if !GAMEMODE.Worlds:GetWorld( v ).ZombieSpawns then continue end
			if !IsValid(v) then continue end
			if !v:Alive() then continue end
			if v.amode then continue end
			if !(v:GetInitialized() == INITSTATE_OK) then continue end
...
function SGS_SpawnZombieBloodMoon()
	if !GAMEMODE.Worlds.BloodMoon then return end
	if SGS_DayPhase() ~= "night" then return end
	...
	for k, v in pairs( player.GetAll() ) do
		...
		if !GAMEMODE.Worlds:GetWorld( v ) then continue end
		if !GAMEMODE.Worlds:GetWorld( v ).ZombieSpawns then continue end
		if !GAMEMODE.Worlds:GetWorld( v ).AllowBloodmoon then continue end
		if !IsValid(v) then continue end
		if !v:Alive() then continue end
		if v.amode then continue end
		if (v:GetInitialized() ~= INITSTATE_OK) then continue end
