...
function PlayerMeta:DropResource2( rType, rAmt )
	if !SGS_CanDropResource(rType) then
...
function PlayerMeta:DropResource( rType, rAmt, own, sub, combine )
	...
	if found and !( found.rAmt == nil ) then
...
function PlayerMeta:DeathDropResources()
	...
	local npos = self:GetPos() +  Vector(0,0,40)
		ent:SetPos( npos )
		ent:Spawn()
	...
	for k, v in pairs(self.resource) do
		...
		if math.random(1,100) + lvlmodi > 1 then
			local amt = math.random( math.ceil( v / 2 ), v )
			if amt > 0 then
				self:SubResource( k, amt, false, true )
				ent:SetResourceDropInfo( k, amt )
				dropamt = amt
			end
		end
	...
	SafeRemoveEntityDelayed( ent , 420 )
...
function SGS_ConTakeResource( ply, com, args )
	...
	if !( tr.Entity:GetClass() == "gms_resourcedrop" ) then
		ply:SendMessage("You need to be looking at a resource drop.", 60, Color(255, 0, 0, 255))
		return false
	end

	if !( rbox:CPPICanTool(ply, true) ) then
