...
function SGS_NamePetCon(ply, com, args)
	...
	if IsValid(ply.lastpet) and (!ply.lastpet == nil) then
...
function SGS_PetDieAge( pet )
	...
	if IsValid(pet.owner) then
		...
		local rtokenamt = math.random(150,300)
...
function SGS_PetFromCage( ply, petid, cage )
	...
	if !IsValid(cage) then return end
...
function PetFromCageConCommand( ply, _, args )
	...
	if !trace.Entity:GetClass() == "gms_pethouse" then
		return
	end
...
function SGS_IncubateEgg( ply, _, args )
	...
	if !string.match( args[1], "pet_egg" ) then return end
