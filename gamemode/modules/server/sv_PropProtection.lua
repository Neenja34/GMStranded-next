...
function SPropProtection.PlayerCanTouch(ply, ent)
	...
	if(!ent:GetNWString("Owner", "") or ent:GetNWString("Owner", "") == "" and !ent:IsPlayer()) then
		...
		for k, v in pairs( ents.FindByClass( "ring_*" ) ) do
		ent:SetNWString("Owner", "World")
		return end
		for k, v in pairs( ents.FindByClass( "dhd_*" ) ) do
		ent:SetNWString("Owner", "World")
		return end
...
function SPropProtection.DRemove(SteamID, PlayerName)
	for k,v in pairs(SPropProtection.Props) do
		if(v.SteamID == SteamID and v.Ent:IsValid()) then
			if !table.HasValue(dontRemove, v.Ent:GetClass()) then
	...
	if(tonumber(SPropProtection.Config["dpd"]) == 1) then
		...
		if ply:IsMember() then
			if ply:IsDonator() then
				timer.Create("SPropProtection.DRemove: "..sid, 345600, 1, function() SPropProtection.DRemove( sid, sname ) end )
			else
				if ply:IsUserGroup("veteran") or ply:IsUserGroup("moderator") then
					timer.Create("SPropProtection.DRemove: "..sid, 259200, 1, function() SPropProtection.DRemove( sid, sname ) end )
				return end
				timer.Create("SPropProtection.DRemove: "..sid, 172800, 1, function() SPropProtection.DRemove( sid, sname ) end )
			end
		else
			timer.Create("SPropProtection.DRemove: "..sid, 3600, 1, function() SPropProtection.DRemove( sid, sname ) end )
		end
	end
end
...
function SPropProtection.PhysGravGunPickup(ply, ent)
	...
	if !SPropProtection.KVcanuse[ent:EntIndex()] then SPropProtection.KVcanuse[ent:EntIndex()] = -1 end
...
function SPropProtection.CanTool(ply, tr, mode)
	...
	elseif(mode == "nail") then
		...
		if !SPropProtection.KVcanuse[tr2.Entity:EntIndex()] then SPropProtection.KVcanuse[tr2.Entity:EntIndex()] = -1 end
...
function SPropProtection.ClearFriends( ply, cmd, args )
	...
	for k,v in pairs( SPropProtection[ ply:SteamID() ] ) do
		RunConsoleCommand( "spp_friend_" .. string.gsub( v, ":", "_" ) .. " 0\n" )
	end
	SPropProtection[ ply:SteamID() ] = {}
