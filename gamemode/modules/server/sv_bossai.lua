...
function SGS_BossSlamMega( loc, ply )
	...
	for k, v in pairs( ents.FindInSphere(loc, 1000) ) do
		if (v:GetClass() == "npc_antlion") and (!v.ispet) then
...
hook.Add("Think", "RemoveManhackGarbage", function()
	if !SGS.nextgarbagecleanup then SGS.nextgarbagecleanup = CurTime() end
...
function SGS_NPCBossDamage( ent, dmginfo )
	...
	if !infl:IsPlayer() then return end
	...
	if ent:GetClass() == "npc_antlionguard" then
		...
		if math.random(35) == 1 then
			if minioncooldown == true then 
			print( "Minion Cooldown Triggered" ) 
		    else
			minioncooldown = true
			timer.Simple( 45, function() minioncooldown = false end )
	...
	--		SGS_TeleportBossArena( ent )
		...
		if ( ent:Health() < (ent:GetMaxHealth() / 2 ) ) and ( !ent:GetNWBool("enraged", false) ) then
			HunterPhase2( ent )
		end
	end
end
