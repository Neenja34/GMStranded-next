function SGS_StartTimer( ply, txt, len, skill )
	...
	if !skill then skill = "generic" end
...
function SGS_Lumber_Stop(ply, tree, lvl, modi, gem, void)
	...
	if ( math.random(1,100) * modi ) * lvlmodi * modifier >= 40 then		
		...
		if ply:GetNWBool("hasquest") == true and ply:GetNWString("Quest") == "Cut Wood." then 
...
function SGS_StoneMine_Stop(ply, stone, modi, gem, void, autosmelt)
	...
	if ( math.random(1,100) * modi ) * lvlmodi >= 30 then		
		...
		local size = math.random(1,100)
		local ent = {}
		if ( !ply:InGrindMode() ) and ( !autosmelt ) then
		...
		if size < 70 then
			if ( !ply:InGrindMode() ) and ( !autosmelt ) then
				if !autosmelt then
					...
					if ply:GetNWBool("hasquest") == true and ply:GetNWString("Quest") == "Mine mineral." then
						ply:SetNWInt("amounttodo", ply:GetNWInt("amounttodo") - 1 )
							if ply:GetNWInt("amounttodo") == 0 then
								ply:RemoveQuest()
								ply:AddResource("quest_token", 1)
...
function SGS_MeteorMine_Stop(ply, res, modi, gem, void)
	...
	if ( math.random(1,100) * modi ) * lvlmodi >= 80 then		
		...
		if !res.durability then res.durability = 10 end
...
function SGS_EatFruit_Stop(ply, lvl, size)
	...
	if lvl == "goldmelon" then
		...
		if ply:GetNWBool("haswquest") == true and ply:GetNWString("Weekly") == "Eat Golden Melons." then
							ply:SetNWInt("wamounttodo", ply:GetNWInt("wamounttodo") - 1 )
							if ply:GetNWInt("wamounttodo") == 0 then
								ply:RemoveWeeklyQuest()
								ply:AddResource("quest_token", 1)
	...
	local thirstrestore = math.floor(SGS.Seeds[ "fruit" ][lvl].thirstheal * size)
	if ply.needs["hunger"] + torestore >= ply.maxneeds then
		ply.needs["hunger"] = ply.maxneeds
	else
		ply.needs["hunger"] = ply.needs["hunger"] + torestore
	end
	if ply.needs["thirst"] + thirstrestore >= ply.maxneeds then
		ply.needs["thirst"] = ply.maxneeds
	else
		ply.needs["thirst"] = ply.needs["thirst"] + thirstrestore
	end
	-- ply:Heal(hptorestore)
	ply:SendNeeds()
	-- print(ply:Health())
...
function SGS_Eat_Stop(ply, food)
	...
	if food.sleepheal then
	local sleepr = food.sleepheal
	if ply.needs["fatigue"] + sleepr >= ply.maxneeds then
		ply.needs["fatigue"] = ply.maxneeds
	else
		ply.needs["fatigue"] = ply.needs["fatigue"] + sleepr
	end
	end
	ply:SendMessage("You feel less hungry!", 60, Color(255, 255, 0, 255))
...
function SGS_Forage_Stop(ply, modi, void)
	...
	if ( math.random( 1, 100 ) * modi ) * lvlmodi >= 45 then
		...
		ply:ConCommand( "sgs_refreshfarming" )
			if ply:GetNWBool( "hasquest" ) and ply:GetNWString( "Quest" ) == "Forage seeds." then
				ply:SetNWInt( "amounttodo", ply:GetNWInt( "amounttodo" ) - 1 )
				if ply:GetNWInt( "amounttodo" ) == 0 then
					ply:RemoveQuest()
					ply:AddResource( "quest_token", 1 )
...
function SGS_Harvest_Tree_Seed(ply, tree, timemodi, modi)

	if !IsValid(tree) then return end
	if tree:GetClass() == "gms_treeseed" then
		ply:SendMessage("Be patient, you can't harvest this yet!", 60, Color(255, 125, 0, 255))
		return
	end

	local rlvl = SGS_LookupResourceTranslation(tree:GetClass()).rlvl
	if ply:GetLevel("woodcutting") < rlvl then
		ply:SendMessage("This tree requires level " .. tostring(rlvl) .. " or greater to harvest seeds from it.", 60, Color(255, 125, 0, 255))
		return
	end

	local newtime = tree.baselen - (timemodi - 0.5)
	SGS_Harvest_Tree_Seed_Start(ply, tree, newtime, tree.lvl, modi)
end

function SGS_Harvest_Tree_Seed_Start(ply, tree, len, lvl, modi)
	...
	if 	ply.equippedtool == "gms_hoe5" or ply.equippedtool == "gms_hoe6" or ply.equippedtool == "gms_hoe7" or 
		ply.equippedtool == "gms_hoe8" or ply.equippedtool == "gms_ehoe" then
		if IsValid(tree.owner) and tree.owner:IsPlayer() then
			ply:SendMessage("You can't harvest player-planted trees!", 60, Color(255, 0, 0, 255))
		else
			...
			ply:SetNWString("action", txt)
			SGS_StartTimer(ply, txt, len, "woodcutting")
			timer.Create(ply:UniqueID() .. "processtimer", len, 1, function() SGS_Harvest_Tree_Seed_Stop(ply, tree, lvl, modi) end)
		end
	end
end

function SGS_Harvest_Tree_Seed_Stop(ply, tree, lvl, modi)
	...
	if tree.lvl == 2 then
		seedtype = "oak_seed"
	elseif tree.lvl == 3 then
		seedtype = "maple_seed"
	elseif tree.lvl == 4 then
		seedtype = "pine_seed"
	elseif tree.lvl == 5 then
		seedtype = "yew_seed"
	elseif tree.lvl == 6 then
		seedtype = "buckeye_seed"
	elseif tree.lvl == 7 then
		seedtype = "palm_seed"
	end
	...
	local removechance = math.random(0,100)
	if removechance <= 65 then
		SGS_RemoveAResource(tree)
		ply:SendMessage("You have exhausted this resource.", 60, Color(255, 255, 255, 255))
		tree:EmitSound("physics/wood/wood_box_break2.wav", 60, math.random(80,120))
	end
...
function SGS_BuildProp_Stop(ply, ent, item)
	...
	if item.xp > 0 then
		...
		if ply:GetNWBool("hasquest") == true and ply:GetNWString("Quest") == "Build props." then
				ply:SetNWInt("amounttodo", ply:GetNWInt("amounttodo") - 1 )
				if ply:GetNWInt("amounttodo") == 0 then
					ply:RemoveQuest()
					ply:AddResource("quest_token", 1)
...
function SGS_UnpackStructure_Start(ply, len, item)
	...
	if GAMEMODE.Worlds:GetEntityWorldSpace( ply ).RestrictUnpack then
		if !table.HasValue( GAMEMODE.Worlds:GetEntityWorldSpace( ply ).AllowedUnpackEntities, item ) then
			ply:SendMessage("Spawning Items in the Arena is prohibited.", 60, Color(255, 0, 0, 255))
			return
		end
	end
...
function SGS_Fish_Start(ply, len, modi, void)
	...
	if !ply:CheckWater(800) then return end
	ply:Freeze( true )
	ply.inprocess = true
...
function SGS_Fish_Stop(ply, modi, void)
	...
	if chance >= 60 then		
		...
		if catch.reqlvl <= ply:GetLevel( "fishing" ) then
			...
			if ply:GetNWBool("hasquest") == true and ply:GetNWString("Quest") == "Fish up fish." then
				ply:SetNWInt("amounttodo", ply:GetNWInt("amounttodo") - 1 )
				if ply:GetNWInt("amounttodo") == 0 then
					ply:RemoveQuest()
					ply:AddResource("quest_token", 1)
...
function SGS_Brew_Stop(ply, potion, modi)
	...
	if chance > 15 then
		...
		ply:AddStat( "alchemy1", 1 )
		if ply:GetNWBool("hasquest") == true and ply:GetNWString("Quest") == "Brew potions." then
				ply:SetNWInt("amounttodo", ply:GetNWInt("amounttodo") - 1 )
				if ply:GetNWInt("amounttodo") == 0 then
					ply:RemoveQuest()
					ply:AddResource("quest_token", 1)
...
function SGS_XMute_Start(ply, len, recipee, modi)
	...
	if !ply.equippedtool == "gms_alchemystone" then
...
function SGS_Grind_Start(ply, len, recipee, modi)
	...
	if !ply.equippedtool == "gms_grindingstone" then
...
function SGS_Smith_Stop(ply, tool, modi)
	...
	if ply:GetNWBool("hasquest") == true and ply:GetNWString("Quest") == "Smith items." then
		ply:SetNWInt("amounttodo", ply:GetNWInt("amounttodo") - 1 )
		if ply:GetNWInt("amounttodo") == 0 then
			ply:RemoveQuest()
			ply:AddResource("quest_token", 1)
...
function SGS_GemTool_Stop(ply, tool, modi)
	...
	if !ply:HasToolInventory( tool.entity2 ) then
		can = false
...
function SGS_DrinkPotion_Start(ply, len, potion)
	...
	if potion.ptype == "upgrade" then
		...
		if potion.uid == "tribe_upgrade" then
			if !GAMEMODE.Tribes:PlayerOwnsTribe( ply ) then
