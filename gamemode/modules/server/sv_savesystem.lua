...
net.Receive( "sgs_readytoload", function( len, ply )
	...
	for _, v in pairs( player.GetAll() ) do
		if ( ply:IsAdmin() and v:IsAdmin() ) or ( !ply:IsAdmin() ) then
...
function PlayerMeta:SaveCharacter()
	self:CheckForAchievements( "all" )
	self:AddExp("mining",0)
	self:AddExp("woodcutting",0)
	self:AddExp("farming",0)
	self:AddExp("fishing",0)
	self:AddExp("smithing",0)
	self:AddExp("cooking",0)
	self:AddExp("construction",0)
	self:AddExp("combat",0)
	self:AddExp("alchemy",0)
	self:AddExp("arcane",0)
	if !self.exp then return end --Don't save a character that isn't fully loaded yet.
	...
	tbl.inventory.pcachemeteoric = self.pmeteoriccontents
...
function SGS_LoadPlayer( ply )
	...
	ply.pmeteoriccontents = tbl.inventory.pcachemeteoric
...
function GM:PlayerDisconnected(ply)
	...
	if !ply:Alive() and !ply:GetNWBool("displaydeathnotice", false) then
		ply:DeathTwo()
	end
...
function SGS_CleanInventories( ply )
	...
	if ply.pmeteoriccontents then
		for k, v in pairs( ply.pmeteoriccontents ) do
			if v <= 0 then ply.pmeteoriccontents[k] = nil end
		end
	end
end