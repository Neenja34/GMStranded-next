...
function meta:SetCrowMode( bEnabled, strModel )
	...
	if !self.m_tblCrowMode then
		self.m_tblCrowMode = { Enabled = false, OldModel = self:GetModel() }
	end

	if bEnabled then
		if !self.m_tblCrowMode.Enabled then
...
local function HandleCrowVaulting( pPlayer, vecVel )
	if pPlayer:IsOnGround() or !Anims[pPlayer:GetModel()] then return end
...
local function HandleCrowLanding( pPlayer, vecVel, bWasOnGround ) 
	if pPlayer:GetMoveType() == MOVETYPE_NOCLIP or !Anims[pPlayer:GetModel()] then return end
	if pPlayer.m_intCrowLandingTime or (pPlayer:IsOnGround() and !bWasOnGround) or pPlayer.m_bWasNoclipping then
		pPlayer.CalcIdeal = Anims[pPlayer:GetModel()].ACT_CROW_LAND
		pPlayer.CalcSeqOverride = Anims[pPlayer:GetModel()].ACT_CROW_LAND

		if !pPlayer.m_intCrowLandingTime then
...
local function HandleCrowNoClipping( pPlayer, vecVel )
	...
	if !pPlayer.m_intNoClipStartTime and !pPlayer.m_bNoClipStarted and pPlayer.m_bWasOnGround then
	...
	elseif pPlayer.m_intNoClipStartTime and CurTime() > pPlayer.m_intNoClipStartTime then
		...
		pPlayer.m_intNoClipStartTime = nil
		pPlayer:AnimRestartMainSequence()
	elseif !pPlayer.m_bWasOnGround and !pPlayer.m_bNoClipStarted and !pPlayer.m_intNoClipStartTime then
		pPlayer.m_bNoClipStarted = true
		pPlayer:AnimRestartMainSequence()
	end
	
	if pPlayer.m_intNoClipStartTime and !pPlayer.m_bNoClipStarted then
...
local function HandleCrowDucking( pPlayer, vecVel )
	if !pPlayer:Crouching() or !Anims[pPlayer:GetModel()] then return false end
...
local function HandleCrowJumping( pPlayer, vecVel )
	...
	if !pPlayer.m_bJumping and !pPlayer:OnGround() and pPlayer:WaterLevel() <= 0 then
		if !pPlayer.m_fGroundTime then
...
hook.Add( "CalcMainActivity", "CrowMode", function( pPlayer, vecVel )
	if !pPlayer:GetCrowMode() then return end
	if !Anims[pPlayer:GetModel()] then return end
	pPlayer.CalcIdeal = Anims[pPlayer:GetModel()].ACT_CROW_IDLE
	pPlayer.CalcSeqOverride = Anims[pPlayer:GetModel()].ACT_CROW_IDLE
	...
	pPlayer.m_bWasNoclipping = ( pPlayer:GetMoveType() == MOVETYPE_NOCLIP and !pPlayer:InVehicle() )
