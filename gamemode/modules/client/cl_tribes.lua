...
net.Receive( "tribes_sv_networkpart", function( l )
	...
	elseif to_edit == 1 then
		...
		GAMEMODE.Tribes.tblTribes[ t_id ].expcontributions[ LocalPlayer():SteamID() ] = new_exp
	end
...
function GM.Tribes:LeaveTribe()
	Derma_Query( "You are about to leave your tribe... are you sure?", "Confirm Tribe Disband",
	"Yes",
	function()
	net.Start( "tribes_cl_jointribe" )
...
function PANEL:Init()
	if !GAMEMODE.Tribes:PlayerInTribe( LocalPlayer() ) then
...
function PANEL:DrawFrame()
	...
	if ( GAMEMODE.Tribes:PlayerInTribe( LocalPlayer() ) ) and LocalPlayer():GetTribe().level > 0 then
		...
		for k, v in pairs( LocalPlayer():GetTribe().members ) do
			...
			if !LocalPlayer():GetTribe().expcontributions then LocalPlayer():GetTribe().expcontributions = {} end
	...
	btn_create.OnMousePressed = function()
		if !btn_create.enabled then return end
	...
	btn_leave.OnMousePressed = function()
		if !btn_leave.enabled then return end
	...
	btn_disband.OnMousePressed = function()
		if !btn_disband.enabled then return end
	...
	btn_achievements.OnMousePressed = function()
		if !btn_achievements.enabled then return end
...
function PANEL:DrawFrame()
	...
	invite_button.OnMousePressed = function()
		if !invite_button.enabled then return end
...
function PANEL:DrawFrame()
	...
	if !(GAMEMODE.Tribes:IsValid( GAMEMODE.Tribes:GetPlayersTribe( LocalPlayer() ) ) and GAMEMODE.Tribes.tblTribes[ GAMEMODE.Tribes:GetPlayersTribe( LocalPlayer() ) ].perms.ptype == 2) then
		btn_invite:Remove()
	end
