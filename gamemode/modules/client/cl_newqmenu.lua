...
function SPELLS_TAB:DrawFrame()
	...
	local function DrawSpellsNew()
		...
		MenuLabel:SetButtonText( "Arcane Spells" )
		MenuLabel:Dock(TOP)
...
function OPTIONS_TAB:DrawFrame()
	function DrawOptionsNew()
		...
		local CrosshairSettingsButton = vgui.Create( "menu_qResourceButton", OptionContainerButtons )
		CrosshairSettingsButton:SetSize( 120, 20 )
		CrosshairSettingsButton:SetButtonText( "Crosshair Settings" )
		CrosshairSettingsButton.SetTextOffsetY = 2
		OptionContainerButtons:Add( CrosshairSettingsButton )
		CrosshairSettingsButton:SetMouseInputEnabled( true )
		function CrosshairSettingsButton:DoClick()
			surface.PlaySound( "ui/buttonclickrelease.wav" )
			OptionContainerButtons.CrosshairDialogue = vgui.Create( "crosshair_dialogue" )
			OptionContainerButtons.CrosshairDialogue:MakePopup()
		end
	end
	DrawOptionsNew()
...
--Crosshair Color Panel

local CrosshairPanel = {}
function CrosshairPanel:Init()
...
function CrosshairPanel:DrawFrame()
	local function DrawCrosshairFrameTick()

		colorpanel = vgui.Create( "DPanel", self )
		colorpanel:Dock( FILL )
		colorpanel:SetPaintBackground( false )

		local title = vgui.Create( "menu_qLabelBar", colorpanel )
		title:SetButtonText( "Crosshair Settings" )
		title:Dock( TOP )
		title:DockMargin( 6, 6, 6, 0 )
		title.PaintColor = Color( 40, 40, 40, 230 )

		local crosshaircolor = vgui.Create( "DColorMixer", colorpanel )
		crosshaircolor:SetColor(Color(255,255,255,255))
		crosshaircolor:Dock( TOP )
		crosshaircolor:DockMargin( 6, 16, 250, 0 )
		crosshaircolor:SetPalette( true )
		crosshaircolor:SetAlphaBar( true )
		crosshaircolor:SetWangs( false )

		local crosshaircolorButtons = vgui.Create( "DIconLayout", colorpanel)
		crosshaircolorButtons:SetSpaceY( 4 )
		crosshaircolorButtons:SetSpaceX( 5 )
		crosshaircolorButtons:Dock( BOTTOM )
		crosshaircolorButtons:DockMargin( 6, 10, 6, 0 )

		local colorset = vgui.Create( "menu_qButton2", crosshaircolorButtons)
		colorset:SetSize( 116, 18 )
		colorset:SetButtonText( "Set Color" )
		colorset.SetTextOffsetY = 2
		colorset.OnMousePressed = function( panel, m )
			if m == MOUSE_LEFT then
				surface.PlaySound( "ui/buttonclickrelease.wav" )
				local color = crosshaircolor:GetColor()
				--RunConsoleCommand( "sgs_setcrosshaircolor", chcolor.r, chcolor.g, chcolor.b, chcolor.a )
				RunConsoleCommand("sgs_crosshair_color", color.r .. "," .. color.g .. "," .. color.b .. "," .. color.a)
			end
		end

		local colorclose = vgui.Create( "menu_qButton2", crosshaircolorButtons)
		colorclose:SetSize( 116, 18 )
		colorclose:SetButtonText( "Close" )
		colorclose.SetTextOffsetY = 2
		colorclose.OnMousePressed = function( panel, m )
			if m == MOUSE_LEFT then
				surface.PlaySound( "ui/buttonclickrelease.wav" )
				self:Remove()
			end
	...
	timer.Simple( 0, DrawCrosshairFrameTick )
end
vgui.Register( "crosshair_dialogue", CrosshairPanel, "EditablePanel" )