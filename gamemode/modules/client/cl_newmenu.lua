...
function PANEL:DrawFrame()
	...
	GAMEMODE.NewMenu.btn_option4 = vgui.Create( "menu_button", button_dock )
	...
	GAMEMODE.NewMenu.btn_option1:SetText( "Website" )
	...
	GAMEMODE.NewMenu.btn_option2:SetText( "Guide" )
	...
	GAMEMODE.NewMenu.btn_option3:SetText( "Achievements" )
	...
	GAMEMODE.NewMenu.btn_option4:SetWide( 130 )
	GAMEMODE.NewMenu.btn_option4:SetText( "Rules" )
	GAMEMODE.NewMenu.btn_option4:DockMargin( 2, 2, 4, 2 )
	GAMEMODE.NewMenu.btn_option4:Dock( TOP )
	GAMEMODE.NewMenu.btn_option4.OnMousePressed = function()
		DeactivateAllButtons()
		GAMEMODE.NewMenu.btn_option4.active = true
		
		if IsValid(self.content) then self.content:Remove() end
		self.content = vgui.Create( "rules_panel", content_window )
		self.content:Dock( FILL )
		self.content:DockMargin( 0, 0, 0, 0 )
	end
	
	GAMEMODE.NewMenu.btn_option5:SetWide( 130 )
	GAMEMODE.NewMenu.btn_option5:SetText( "Content" )
	GAMEMODE.NewMenu.btn_option5:DockMargin( 2, 2, 4, 2 )
	...
	GAMEMODE.NewMenu.btn_option5.OnMousePressed = function()
		...
		if IsValid(self.content) then self.content:Remove() end
		self.content = vgui.Create( "scontent_panel", content_window )
		self.content:Dock( FILL )
		self.content:DockMargin( 0, 0, 0, 0 )
	end
	
	GAMEMODE.NewMenu.btn_option6:SetWide( 130 )
	GAMEMODE.NewMenu.btn_option6:SetText( "Donate" )
...
function DeactivateAllButtons()
	...
	GAMEMODE.NewMenu.btn_option4.active = false
...
function MOTD:Init()
	...
	HTML:Dock( FILL )
	HTML:OpenURL("http://neenja.nn.pe/")
end
function MOTD:Paint()
...
function WIKI:Init()
	local HTML = vgui.Create("DHTML", self)
	HTML:Dock( FILL )
	HTML:OpenURL("http://neenja.nn.pe/guide")
end
function WIKI:Paint()

end
vgui.Register("guide_panel", WIKI, "Panel")

local GUIDE = {}
function GUIDE:Init()
	local HTML = vgui.Create("DHTML", self)
	HTML:Dock( FILL )
	HTML:OpenURL("http://neenja.nn.pe/rules")
end
function GUIDE:Paint()

end
vgui.Register("rules_panel", GUIDE, "Panel")

local SCONTENT = {}
function SCONTENT:Init()
	local HTML = vgui.Create("DHTML", self)
	HTML:Dock( FILL )
	HTML:OpenURL("https://steamcommunity.com/sharedfiles/filedetails/?id=1690678237")
end
function GUIDE:Paint()

end
vgui.Register("scontent_panel", SCONTENT, "Panel")
...
function DONATE:Init()
	local HTML = vgui.Create("DHTML", self)
	HTML:Dock( FILL )
	HTML:OpenURL("http://neenja.nn.pe/donate/store.php?page=packages&id=1")
end
function GUIDE:Paint()
