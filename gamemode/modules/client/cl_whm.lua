...
function WHM:Start( intTypeID, intTime, intTimeOffset )
	if !self:ValidType( self:GetType(intTypeID) ) then print "wat" return end

	if !self:IsTypeActive( intTypeID ) then
...
function WHM:Stop( intTypeID )
	if !self:ValidType( self:GetType(intTypeID) ) then return end
	if !self:IsTypeActive( intTypeID ) or self.m_tblActiveTypes[intTypeID].Stopping then return end
