...
function SkyObj.CBaseStellarObject:GetParentBody()
	if self.m_strParentName and !self.m_pParentBody then
...
function SkyObj.CBaseStellarObject:Tick()
	if !self.m_tblOrbitData.Radius then return end
	local data = self.m_tblOrbitData
	local day = self.m_tblDayData

	if self:GetParentBody() then
		if !day.StartTime then
			return
		end
	...
	elseif !self.m_intElevationOverride then
		if !day.StartTime then
			return
		end
...
function SkyObj.CBaseStellarObject:Render()
	if !self.m_tblDayData.StartTime then return end
